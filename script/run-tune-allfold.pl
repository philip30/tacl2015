...
my $stat_gen_options = "$LETRAC/stat-generator.py -working_dir $WORKING_DIR -driver_function $DRIVER_FUNCTION -letrac $LETRAC -geoquery \\\"swipl -s $GEOQUERY\\\" -database_config $DATABASE $no_typecheck $check_empty";
safesystem("script/tune/folds-travatar.pl -lamtram-dir $LAMTRAM -nbest $N_BEST -travatar-config $CONFIG -travatar-dir $TRAVATAR -working-dir $WORKING_DIR -trace -threads $THREADS -trg-factors $TRG_FACTORS -fold-config $FOLD_CONFIG -stat-generator \"$stat_gen_options\" $tune_options $nnlm") or die;
