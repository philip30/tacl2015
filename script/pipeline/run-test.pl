...
if (not (defined ($WORKING_DIR) && defined($TRAVATAR) && defined($GEOQUERY) 
        && defined ($LETRAC) && defined($SRC) && defined($REF) && defined($NL_MRL_CONFIG) && defined($KW_NL_CONFIG)
        && defined ($TRG_FACTORS) && defined($DATABASE) && defined($DRIVER_FUNCTION)
        && defined ($LAMTRAM))) 
{
    die "Usage run-test.pl -working-dir [WD] -geoquery [GEOQUERY] -travatar [TRAVATAR] -kwnl-config [CONFIG] -nlmrl-config [CONFIG] -src [SRC] -ref [REF] -trg-factors [TRG_FACTORS] -letrac [LETRAC] -database-config [DATABASE] -driver-function [DRIVER_FUNCTION] -lamtram [LAMTRAM]"; 
}
if (-d $WORKING_DIR) {
    if ($FORCE) {
...
### Execute decoding
safesystem("cp $KW_NL_CONFIG $WORKING_DIR/kw-nl.ini") or die;
safesystem("cp $NL_MRL_CONFIG $WORKING_DIR/nl-mrl.ini") or die;

# PIPELINED DECODING
# 1. Translate Keyword into Natural Language
if ($NNLM_CONFIG) {
...
if ($NNLM) {
    ...
    safesystem("script/tune/nnlm-rescore.py --score $WORKING_DIR/scores.ppl --input $WORKING_DIR/nbest-para.txt --ini $WORKING_DIR/travatar-nnlm.ini --output $WORKING_DIR/paraphrase.txt --extract_one_best") or die; 
}
