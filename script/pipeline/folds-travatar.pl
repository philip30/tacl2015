...
foreach my $iter (1 .. $MAX_ITERS) {
    ...
    foreach my $n (sort keys %fold_config) {
        ...
        safesystem(sprintf("$TRAVATAR -threads $THREADS $format $trace -trg_factors $TRG_FACTORS -config_file $prev.ini -tm_file \"%s|%s\" $DECODER_OPTIONS $CAND_OPTIONS $lm_file < %s > $prev-fold-$n.out 2> $prev-fold-$n.err", $fold_config{$n}->{"tm_file"}, $fold_config{$n}->{"glue"}, $fold_config{$n}->{"src"})) or die "couldn't decode";
    ...
    safesystem("$TRAVATAR_DIR/src/bin/batch-tune -trg_factors $TRG_FACTORS -threads $THREADS -nbest $WORKING_DIR/run$iter.uniq -stat_out $WORKING_DIR/run$iter.stats -eval \"bleu\" $REF 2> $prev.stats.log") or die "batch-tune stats extraction failed";
    
    $nbests = join(",", map { "$WORKING_DIR/run$_.uniq" } (1 .. $iter));
    my $stats = join(",", map { "$WORKING_DIR/run$_.stats" } (1 .. $iter));
    safesystem("$TRAVATAR_DIR/src/bin/batch-tune -threads $THREADS -nbest $nbests -stat_in $stats -eval \"bleu\" -weight_in $prev.weights $TUNE_OPTIONS $REF > $next.weights 2> $prev.tune.log") or die "batch-tune failed";
...
if ($NNLM) {
    ...
    safesystem("$TRAVATAR_DIR/src/bin/batch-tune -threads $THREADS -nbest $WORKING_DIR/run$iter.uniq -stat_in $WORKING_DIR/run$iter.stats -eval \"bleu\" -weight_in $prev.weights $TUNE_OPTIONS $REF > $next.weights 2> $prev.tune.log") or die "batch-tune failed";
    safesystem("$TRAVATAR_DIR/script/mert/update-weights.pl -weights $next.weights $prev.ini > $next.ini") or die "couldn't make init opt";
...
sub load_fold_config {
    ...
    foreach my $config_file (@config_files) {
        ...
        while(<FILE0>) {
            ...
            last if not $_;
            my ($k, $v) = split(/=/);
            if (not $k =~ /^(src|ref|tm_file|lm_file|glue)/) {
                close FILE0;
